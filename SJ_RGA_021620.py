...
def render_mpl_table(data, col_width=3.0, row_height=0.5, font_size=8, header_color='#40466e',
        row_colors=['#f1f1f2', 'w'], edge_color='w', bbox=[0, 0, 1, 1], header_columns=0, ax=None, **kwargs):
        if ax is None:
            size = (np.array(data.shape[::-1]) + np.array([0, 1])) * np.array([col_width, row_height])
            fig, ax = plt.subplots(figsize=size)
            ax.axis('off')
        mpl_table = ax.table(cellText=data.values, bbox=bbox, cellLoc='left', colLabels=data.columns, **kwargs)
...
def SJHD_TickRprtDF():
    ...
    df['Short description'] = df['Short description'].apply(lambda a: char_len(a, 80))
    return df
...
def SJHD_TickRprtEsc(imageName):
    df = SJHD_TickRprtDF()
    Helpdesk = 'IS 4th Source Helpdesk'
    Esc = df[(df['Assignment group'] != Helpdesk)]
    render_mpl_table(Esc, colWidths=[2.5, 10.0, 5.5])
    plt.savefig("File_{}.png".format(imageName))
    plt.show()

def SJHD_TickRprtRes(imageName):
    df = SJHD_TickRprtDF()
    Helpdesk = 'IS 4th Source Helpdesk'
    Res = df[(df['Assignment group'] == Helpdesk)]
    render_mpl_table(Res, colWidths=[2.5, 10.0, 5.5])
    plt.savefig("File_{}.png".format(imageName))
    plt.show()

def SJM_TickRprtEsc(imageName):
    df = SJM_TickRprtDF()
    Helpdesk = 'IS 4th Source Helpdesk'
    Esc = df[(df['Assignment group'] != Helpdesk)]
    render_mpl_table(Esc, colWidths=[2.5, 10.0, 5.5])
...
def main():
    ...
    #create EscRes graph
    EscRes('SJHD.csv', 'St. Jude Helpdesk\nEscalated & Resolved Graph', 'SJHD_1')
    ...
    #create ticket report tables
    SJHD_TickRprtEsc('SJHD_TickRprtEsc')
    SJHD_TickRprtRes('SJHD_TickRprtRes')
    SJM_TickRprtEsc('SJM_TickRprtEsc')
    SJM_TickRprtRes('SJM_TickRprtRes')
    ...
    #create SJHD_Cat pie chart
    SJHD_Cat('SJHD_7')

    #create SJHD_SubCat pie chart
    SJHD_SubCat('SJHD_8')
    ...
    def createDoc_SJHD():
        ...
        document.add_picture('File_SJHD_TickRprtEsc.png')
        document.add_picture('File_SJHD_TickRprtRes.png')
    ...
    def createDoc_SJM():
        ...
        document.add_heading('Escalated/Resolved:', level=3)
        document.add_picture('File_SJM_TickRprtEsc.png')
        document.add_picture('File_SJM_TickRprtRes.png')
        document.add_heading('Call Statistics:', level=3)

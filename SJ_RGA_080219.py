...
def EscRes(fileName):
    ...
    plt.title('St. Jude Helpdesk\nEscalated & Resolved Graph')
    plt.legend()
    plt.show()

def DailyTicketCount(fileName):
    ...
    plt.subplots_adjust(left=.2, bottom=.25, right=None, top=None, wspace=None, hspace=None)
    plt.show()

def AsgmtGroup(fileName):
...
def CallStat1(fileName, graphTitle):
    df = pd.read_csv(fileName, delimiter=',')
    #find total calls
...
def CallStat2(Iteration, fileName, graphTitle, CSxlabels):
    df = pd.read_csv(fileName, delimiter=',')
    #find total calls
    totalCalls = find_total_calls(df)
    #turn dataframe to list & clean list
...
def main():
    ...
    #define SJHD buttons
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(EscRes('SJHD.csv')))
    b1.grid(row=1,column=2)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(DailyTicketCount('SJHD.csv')))
    b1.grid(row=2,column=2)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(AsgmtGroup('SJHD.csv')))
    b1.grid(row=3,column=2)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(SJHD_TickReports()))
    b1.grid(row=4,column=2)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(CallStat1('SJ Help Desk - Previous Week.csv',
                                                                                                                  'St. Jude Help Desk\nCall Statistics\n')))
    b1.grid(row=5,column=2)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(CallStat2(8,
                                                                                                                  'SJ Help Desk - Previous Week.csv','St. Jude Help Desk\nCall Statistics (Averages)\n',
                                                                                                                  ['Answer Speed', 'After Call Work Time', 'Call  Time', 'Handle Time', 'Hold Time', 'Queue Wait Time', 'Ring Time', 'Talk Time'])))
    b1.grid(row=6,column=2)

    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(SJHD_Cat()))
    b1.grid(row=7,column=2)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(SJHD_SubCat()))
    b1.grid(row=8,column=2)

    #define SJM buttons 
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(EscRes('SJM.csv')))
    b1.grid(row=1,column=4)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(DailyTicketCount('SJM.csv')))
    b1.grid(row=2,column=4)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(AsgmtGroup('SJM.csv')))
    b1.grid(row=3,column=4)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(SJM_TickReports()))
    b1.grid(row=4,column=4)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(CallStat1('SJ Milli - Previous Week.csv',
                                                                                                                  'St. Jude Milli Support\nCall Statistics\n')))
    b1.grid(row=5,column=4)
    b1 = Button(window,text="Generate:", width=12, height=2, bg='cyan', fg='black', command=lambda: run(CallStat2(7,
                                                                                                                  'SJ Milli - Previous Week.csv',

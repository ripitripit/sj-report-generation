...
def render_mpl_table(data, col_width=6.0, row_height=1.5, font_size=12, header_color='#40466e',
...
def SJM_TickRprtDF():
    ...
    df['Resolution notes'] = df['Resolution notes'].fillna('N/A')
    df['Resolution notes'] = df['Resolution notes'].apply(lambda a: char_len(a, 90))
    return df
...
def SJM_TickRprtEsc(imageName):
    ...
    render_mpl_table(Esc, colWidths=[2.5, 15.0, 5.5, 20.0])
    plt.savefig("File_{}.png".format(imageName))
    plt.show()
...
def SJM_TickRprtRes(imageName):
    ...
    render_mpl_table(Res, colWidths=[2.5, 15.0, 5.5, 20.0])
    plt.savefig("File_{}.png".format(imageName))
    plt.show()
